...
namespace DummyExtensions.MongoDB.Externals
{
    ...
    public class JsonWriter : BsonWriter
    {
		...
		private BsonSettings bson_settings;
    	...
    	/// <param name="bson_settings"> </param>
    	public JsonWriter( TextWriter writer, JsonWriterSettings settings, BsonSettings bson_settings )
            : base(settings)
        {
    		_textWriter = writer;
			...
			this.bson_settings = bson_settings;
        }
            ...
            if (_jsonWriterSettings.OutputMode == JsonOutputMode.Shell)
            {
                ...
                switch (subType)
                {
                    ...
                    case BsonBinarySubType.UuidStandard:
                        validate_uuid( bytes, subType, guidRepresentation );
    	...
    	void validate_uuid( byte [ ] bytes, BsonBinarySubType sub_type, GuidRepresentation guid_representation ) {
			if (bytes.Length != 16){
